...
def joint_ratio_plot(lab):
    ...
    for i in range(0,snapshot_num):
        ...
        if lab == 'Velocity Dispersion':
            if galaxy_z[i].bin_centers[n][-1] > 1.75:
                temp = plt.plot(galaxy_z[i].bin_centers[n][:-1],galaxy_z[i].n2[:][n][:-1]/galaxy_z[i].n1[:][n][:-1],'--',label = 'z = '+ str(redshifts[i]))
                plt.scatter(galaxy_z[i].bin_centers[n][:-1],galaxy_z[i].n2[:][n][:-1]/galaxy_z[i].n1[:][n][:-1],s = 6)
            else:
                temp = plt.plot(galaxy_z[i].bin_centers[n],galaxy_z[i].n2[:][n]/galaxy_z[i].n1[:][n],'--',label = 'z = '+ str(redshifts[i]))
    ...
    plt.ylabel('Galaxy Number Density Ratio',fontsize = 17)
...
for i in range(0,snapshot_num):
    ...
    file_write.append(open('/mnt/zfsusers/orie3568/Progen_Plots_2/progen_'+file_string+'761_'+str(snapshots[i][1])+'.dat','w'))
galaxy_761_file = open('/mnt/zfsusers/orie3568/Galaxy_Data/sample_full_761.dat','r')
...
galaxy_761.data_array = np.zeros([galaxy_761.list_length,16])
...
for i in range(0,snapshot_num):
    ...
    for j,index in enumerate(galaxy_761.data_array[:,0],0):
        ...
        #its fine to not fill this plot array because zeros are shavaed off afterwards
