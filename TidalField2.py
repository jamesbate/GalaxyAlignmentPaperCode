...
def TidalField2(masscut_array,vdcut_array,snapshots_array,smoothness_array,name_761_file,cutname = '',masscutbool = False,vdcutbool = False,level_lock = True,wspin = False):
    ...
    class galaxy:
        def __init__(self,snap):
            self.snapshot = snap
            return
    #classes for galaxy data and tidal field data

    def reducer_761(galaxy,lvlone,string,*argv):
        """
        This function applies the required cits to the z =  0
        (snapshot 761) file
        """
        ...
        for [cut_lower,cut_upper,columns] in argv[0]:
            ...
            level_index = level_lock_index - 1 
            ...
            if lvlone == True:
                ...
                #use mask to choose desired data
            else:
                mask = np.logical_and(galaxy.data_array[:,columns] > cut_lower,galaxy.data_array[:,columns] < cut_upper)
                galaxy.data_array = galaxy.data_array[mask]
        ...
        if galaxy.data_array.size == 0:
            ...
            np.savetxt(Galaxy_Data_dir+'/sample_full_'+string+'_761.dat',galaxy.data_array[:,0:level_lock_index], delimiter=" ")
    ...
    def progen_cutter(galaxy_object,progen_file):
        """
        This applies cut to progenitor data in memory to match cut done by merger code
        """
    ...
    def line_finder(pos):
        """
        converts index to nearest pixel. 0th order approximation, no linear interpolation
        over the grid. This could be an extension. 
        """
        pos += 50/0.704
    ...
    def find_angle(tidal_eigenvector,minor):
        """
        return angle between two vectors. If obtuse, make negative
        """
    ...
    start = time.time()
    #time length of time to read files
    ...
    for i in range(0,tidal_num):
        ...
        #This section of code organises axactly what data needs to be cut and where according 
        #to the boolean flags. It is ugly, but it allows the code to be very versitile and perform
        #all the plots needed for the publication
